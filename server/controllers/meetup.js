...
const config = require('config')['Meetups'];
...
module.exports.getAll = (req, res) => {
  models.User.where({ email: req.user.email }).fetch()
    .then((result) => {
      ...
      .then((data) => {
        request.get(`https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyCNXCi32K64d4xFbWvx5jfTQSFnxmbaQCs&address=${result.attributes.destination}`,
              (error, response, body) => {
                if (error) {
                  console.error(err);
                } else {
                  var geoCoords = JSON.parse(body).results[0].geometry['location'];
                  request.get(`https://api.meetup.com/2/concierge?sign=true&photo-host=secure&fields=group_photo&lon=${geoCoords.lng}&lat=${geoCoords.lat}&key=${config.clientID}`, 
                    (error, response, body) => {
                      if (error) {
                        console.error(error);
                      }
